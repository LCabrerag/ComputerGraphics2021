...
int draw_face(face to_draw, color* lines, color* face,raster* output){
    ...
    for ( i = 0; i < pixels.qpixel; i++){
        add_point(output,pixels.position[i],&pixels.color[i], pixels.zbuffer[i]);
    }
}
        ...
        for ( j = 0; j < pixels_face; j++){
            if (destination->position[i].y == destination->position[j].y && destination->position[i].x < destination->position[j].x){       
                ...
                for (l = 0; l < destination->position[j].x - destination->position[i].x; l++){
                    ...
                    if (!part_of_line){
                        zbuffer = destination->zbuffer[i] + ((destination->zbuffer[j]-destination->zbuffer[i])/(destination->position[j].x - destination->position[i].x))*l;
                        //printf("%f\n",zbuffer);
                        scanline.y = destination->position[i].y;
                        scanline.x = destination->position[i].x + l;
                        add_point(output,scanline,background,zbuffer);
                    }
                }
            }
...
int draw_edge(figure* list, color* line_color, edge to_add){
    matrix pointa;
    matrix pointb;
    double zbuffer_pointa;
    double zbuffer_pointb;
    ...
    switch (focal){
        case X_FOCUS:        
            ...
            zbuffer_pointa = (double)pointa_transformed.values[0][0];
            zbuffer_pointb = (double)pointb_transformed.values[0][0];
            break;
        case Y_FOCUS:
            ...
            eline.y = pointb.values[2][0];
            zbuffer_pointa = (double)pointa_transformed.values[1][0];
            zbuffer_pointb = (double)pointb_transformed.values[1][0];
            break;
        case Z_FOCUS:
            ...
            zbuffer_pointa = (double)pointa_transformed.values[2][0];
            zbuffer_pointb = (double)pointb_transformed.values[2][0];
            break;
        default:
    ...
    bre_getline(list, bline, eline, line_color,zbuffer_pointa,zbuffer_pointb);
...
void init_figure(figure* toinit){
    ...
    toinit->zbuffer = (double*) calloc(1, sizeof(double));
}