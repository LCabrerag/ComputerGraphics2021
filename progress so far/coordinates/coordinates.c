...
int add_point(raster* output, coordinate pos, color* newcol, double zbuffer){
    if ((pos.x >= 0 && pos.y >= 0 && pos.x < output->x && pos.y < output->y) ){
        
        if (zbuffer > 0 && ((output->img + GET_POS_XY(pos.x,pos.y,output))->z_buffer == 0 || (output->img + GET_POS_XY(pos.x,pos.y,output))->z_buffer >= zbuffer) ){ 
            (output->img + GET_POS_XY(pos.x,pos.y,output))->pixel_color = *newcol;
            (output->img + GET_POS_XY(pos.x,pos.y,output))->z_buffer = zbuffer;
            
        }else{
            printf("%f %f en %d %d \n", zbuffer, (output->img + GET_POS_XY(pos.x,pos.y,output))->z_buffer, pos.x,pos.y);
        }
    }
}
...
void add_point_to_face(figure* list, color* col_toadd, coordinate cor_toadd, double zbuffer){
    list->color = (color*) realloc(list->color, ((list->qpixel)+1)*sizeof(color));
    list->position = (coordinate*) realloc(list->position, ((list->qpixel)+1)*sizeof(coordinate));
    list->zbuffer = (double*) realloc(list->zbuffer, ((list->qpixel)+1)*sizeof(double));
    
    list->color[list->qpixel] = *col_toadd;
    
    list->position[list->qpixel].x = cor_toadd.x;
    list->position[list->qpixel].y = cor_toadd.y;

    list->zbuffer[list->qpixel] = zbuffer;
    list->qpixel++;
}

void vector_add(vector* A, vector* B ,vector* out){
    out->i = A->i + B->i;
    out->j = A->j + B->j;
    out->k = A->k + B->k;
}

void vector_crossmult(vector* A, vector* B ,vector* out){
    out->i = A->j*B->k - A->k*B->j;
    out->j = A->k*B->i - A->i*B->k;
    out->k = A->i*B->j - A->j*B->i;
}

float vector_dotmult(vector* A, vector* B){
