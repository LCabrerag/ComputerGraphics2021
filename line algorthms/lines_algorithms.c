...
//corrct naive method
void* nai_getline(image* output, coordinate pointa, coordinate pointb, color* linecolor){
    coordinate tmp;
    ...
    if (pointa.x-pointb.x == 0){
        if (pointa.y < pointb.y){
            ...
            for ( i = pointa.y; i < pointb.y+1; i++){
                tmp.x = pointa.x;
                tmp.y = i;
                add_point(output,tmp,linecolor);
            }
        }else{
...
void* dda_getline(image* output, coordinate pointa, coordinate pointb, color* linecolor){
    float m;
    int swap = 0, mirror = 0;
    int size;
    coordinate tmp;
    ...
    for (i = 0; i < size; i++){
        add_point(output,*(line + i),linecolor);
    }
}
void* bre_getline(image* output, coordinate pointa, coordinate pointb, color* linecolor){
    int swap = 0, mirror = 0;
    int size;
    coordinate tmp;
    coordinate *line;
...
//creates threads
int par_nai_getline(image* output, coordinate pointa, coordinate pointb, color* linecolor){
    struct args my_args;
    coordinate tmp_beg;
    coordinate tmp_end;
    int i;
...
int par_dda_getline(image* output, coordinate pointa, coordinate pointb, color* linecolor){

}
int par_bre_getline(image* output, coordinate pointa, coordinate pointb, color* linecolor){
