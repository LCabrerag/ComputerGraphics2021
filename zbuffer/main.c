...
int main(int argc, char** argv){
    if (argc > 2){
        ...
        for (i = 0; i < images; i++){
            ...
            fscanf(toread, "%s", buffer);
            background_model.R = strtol(buffer, NULL, 10);
            fscanf(toread, "%s", buffer);
            background_model.G = strtol(buffer, NULL, 10);
            fscanf(toread, "%s", buffer);
            background_model.B = strtol(buffer, NULL, 10);
            ...
            fscanf(toread, "%s", buffer);
            read_obj(buffer, &output, &line, &background_model, &transformation, focus, f);
            fscanf(toread, "%s", buffer);
